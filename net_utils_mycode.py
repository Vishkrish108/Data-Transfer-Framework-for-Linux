...
class Server:
    ...
    def shutdown_server(self):
        ...
        print('ALERT: inital connections are ', initial_connections/2)
        ...
        while not self.shutdown_complete.is_set():
            with self.lock:
                if self.active_connections == 0:
                    print("[ALERT] ALL CLIENTS CLOSED")
                    self.shutdown_complete.set()
                    break
                else:
                    print(f"Waiting for all clients to close. {self.active_connections} remaining")
                if hasattr(self, 'client_addresses'):
                        print("TEST CHECK")
                        for addr in self.client_addresses:
                            with self.lock:  
